...
上の例は英語表示で保存したもの．日本語表示で保存すると「DELETE」の中身は「削除」になる．
これは行の削除ボタンの表示がそのまま入るためで，設定の中身は変わらず，どちらのファイルも読み込める．

保存した設定を復元するには，「読込」の右側にある「ファイルを選ぶ」ボタンをクリックして，保存したファイルを選択する．
...
- DELETE: 行の削除ボタンの列(設定自体には関係なし)．ボタンの表示は「削除」   
...
「Add species to list」の右にある「ファイルを選ぶ」でファイル名を選択し，「開く」をクリックする．
