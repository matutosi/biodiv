...
- **案2 (D1〜D4) が未コミット・未公開**．`main` は案1 までしか入っていない．
  D4 で見つけた実バグ2件 (**設定の読込が設定表を2つに増やす**，
  **「表を非表示」が非表示列の一覧を残す**) がまだ配布物に反映されていない．
  変更は js2 の 11 ファイル + `biodiv2.html` + 新規 `columns.js`，
  それにテスト2ファイルと `biss2.html`．
